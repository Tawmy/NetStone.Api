...
public class FreeCompanyServiceV4(
    INetStoneService netStoneService,
    IFreeCompanyCachingService cachingService,
    IFreeCompanyEventService eventService,
    ILogger<FreeCompanyServiceV4> logger)
    : IFreeCompanyServiceV4
{
    private static readonly ActivitySource ActivitySource = new(nameof(IFreeCompanyServiceV4));
    ...
    public async Task<FreeCompanyDto> GetFreeCompanyAsync(string lodestoneId, int? maxAge, FallbackTypeV4 useFallback)
    {
        using var activity = ActivitySource.StartActivity();
        ...
        {
            if (cachedFcDto is not null && (useFallback.HasFlag(FallbackTypeV4.Any) ||
        ...
        FallbackTypeV4 useFallback)
    {
        ...
        {
            if (cachedMembers.Any() && (useFallback.HasFlag(FallbackTypeV4.Any) ||
                                        (useFallback.HasFlag(FallbackTypeV4.Http) && ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
    ...
    : IFreeCompanyServiceV3
{
    ...
    public async Task<FreeCompanyDto> GetFreeCompanyAsync(string lodestoneId, int? maxAge, FallbackTypeV3 useFallback)
    {
        using var activity = ActivitySource.StartActivity();
        ...
        {
            if (cachedFcDto is not null && (useFallback is FallbackTypeV3.Any ||
        ...
        FallbackTypeV3 useFallback)
    {
        ...
        {
            if (cachedMembers.Any() && (useFallback is FallbackTypeV3.Any ||
                                        (useFallback is FallbackTypeV3.Http && ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
                    nameof(GetFreeCompanyMembersAsync), ex.Message);
