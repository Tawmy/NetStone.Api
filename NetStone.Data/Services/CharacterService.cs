...
public class CharacterServiceV4(
    ...
    ILogger<CharacterServiceV4> logger)
    : ICharacterServiceV4
{
    private static readonly ActivitySource ActivitySource = new(nameof(ICharacterServiceV4));
    ...
    public async Task<CharacterDto> GetCharacterAsync(string lodestoneId, int? maxAge, FallbackTypeV4 useFallback)
    {
        ...
        {
            if (cachedCharacterDto is not null && (useFallback.HasFlag(FallbackTypeV4.Any) ||
                                                   (useFallback.HasFlag(FallbackTypeV4.Http) &&
        ...
        if (!lodestoneCharacter.IsFullyPublic())
        {
            ...
            if (cachedCharacterDto is not null &&
                (useFallback & (FallbackTypeV4.ProfilePrivate | FallbackTypeV4.Any)) != 0)
            {
        ...
        FallbackTypeV4 useFallback)
    {
        ...
        {
            if (cachedClassJobsDtos.Any() && (useFallback.HasFlag(FallbackTypeV4.Any) ||
                                              (useFallback.HasFlag(FallbackTypeV4.Http) && ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
        ...
        {
            if (cachedClassJobsDtos.Any() &&
                (useFallback & (FallbackTypeV4.LodestoneUnavailable | FallbackTypeV4.Any)) != 0)
            {
        ...
        FallbackTypeV4 useFallback)
    {
        ...
        {
            if (cachedMinionsDtos.Any() && (useFallback.HasFlag(FallbackTypeV4.Any) ||
                                            (useFallback.HasFlag(FallbackTypeV4.Http) && ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
        ...
        if (!lodestoneMinions.Collectables.Any() && cachedMinionsDtos.Any())
        {
            ...
            if ((useFallback & (FallbackTypeV4.LodestoneUnavailable | FallbackTypeV4.Any)) != 0)
            {
                return new CollectionDto<CharacterMinionDto>(cachedMinionsDtos, true, lastUpdated,
        ...
        FallbackTypeV4 useFallback)
    {
        ...
        {
            if (cachedMountsDtos.Any() && (useFallback.HasFlag(FallbackTypeV4.Any) ||
                                           (useFallback.HasFlag(FallbackTypeV4.Http) && ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
        ...
        FallbackTypeV4 useFallback)
    {
        ...
        {
            if (cachedAchievementsDtos.Any() && (useFallback.HasFlag(FallbackTypeV4.Any) ||
                                                 (useFallback.HasFlag(FallbackTypeV4.Http) &&
                                                  ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
    ...
    : ICharacterServiceV3
{
    ...
    public async Task<CharacterDto> GetCharacterAsync(string lodestoneId, int? maxAge, FallbackTypeV3 useFallback)
    {
        using var activity = ActivitySource.StartActivity();
        ...
        {
            if (cachedCharacterDto is not null && (useFallback is FallbackTypeV3.Any ||
                                                   (useFallback is FallbackTypeV3.Http && ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
                    nameof(GetCharacterAsync), ex.Message);
                return cachedCharacterDto with { Cached = true, FallbackUsed = true, FallbackReason = ex.Message };
        ...
        FallbackTypeV3 useFallback)
    {
        ...
        {
            if (cachedClassJobsDtos.Any() && (useFallback is FallbackTypeV3.Any ||
                                              (useFallback is FallbackTypeV3.Http && ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
                    nameof(GetCharacterClassJobsAsync), ex.Message);
        ...
        FallbackTypeV3 useFallback)
    {
        ...
        {
            if (cachedMinionsDtos.Any() && (useFallback is FallbackTypeV3.Any ||
                                            (useFallback is FallbackTypeV3.Http && ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
                    nameof(GetCharacterMinionsAsync), ex.Message);
        ...
        if (!lodestoneMinions.Collectables.Any() && cachedMinionsDtos.Any())
        {
            // no minions returned, but minions were cached before -> Lodestone under maintenance or profile private
            // we cannot always throw when no minions are returned, as new character might actually have none
            if (useFallback is FallbackTypeV3.Any)
            {
                return new CollectionDto<CharacterMinionDto>(cachedMinionsDtos, true, lastUpdated,
                    await collectionData.GetTotalMinionsAsync(), true, nameof(ParsingFailedException));
            }
        ...
        FallbackTypeV3 useFallback)
    {
        ...
        {
            if (cachedMountsDtos.Any() && (useFallback is FallbackTypeV3.Any ||
                                           (useFallback is FallbackTypeV3.Http && ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
                    nameof(GetCharacterMountsAsync), ex.Message);
        ...
        FallbackTypeV3 useFallback)
    {
        ...
        {
            if (cachedAchievementsDtos.Any() && (useFallback is FallbackTypeV3.Any ||
                                                 (useFallback is FallbackTypeV3.Http && ex is HttpRequestException)))
            {
                logger.LogWarning("Fallback used for ID {id} in {method}: {msg}", lodestoneId,
                    nameof(GetCharacterAchievementsAsync), ex.Message);
